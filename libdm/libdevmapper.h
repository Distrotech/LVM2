 ...
 *
 * 	DM_REPORT_RESERVED_PARSE_FUZZY_NAME_AND_GET_DYNAMIC_VALUE
 * 		data_in:  const char *fuzzy_name
 * 		data_out: void *value, NULL if fuzzy_name not recognized
 *
 * All actions return:
 *
...
/*
 * Enable caching of reserved values.
 *
 * This makes it possible for various dm_report_reserved_value_handler
 * instances to cache some information to avoid multiple reevaluation
 * and hence save resources.
 */
int dm_report_reserved_cache_enable(struct dm_report *rh);

/*
 * Disable caching of reserved values.
 *
 * This will cause any cache store and retrieve actions in various
 * dm_report_reserved_value_handler instances to be void operations,
 * hence causing the cache to be bypassed and values always reevaluated.
 * Calling this function also wipes any existing cache records.
 */
int dm_report_reserved_cache_disable(struct dm_report *rh);

/*
 * The dm_report_reserved_cache_{set,get} are* called in
 * dm_report_reserved_handler instances to store values in
 * cache and then to retrieve these values to avoid possible
 * time-consuming recalculations that may appear if the value
 * is reused.
 *
 * To invalidate individual cache record named "name", set
 * "data" to NULL for the dm_report_reserved_cache_set
 * function call.
 */
