 ...
 *
 * The "skip_selection" makes processing function to skip checking
 * the selection criteria (if given on cmd line) before executing
 * the action on the item.
 *
 * The "selection_handle" is only used if "skip_selection=0".
 *
 ...
 * "skip_selection=1" as selection is already a part of that reporting
 * for display (dm_report_object).
 */
...
int select_match_lv(struct cmd_context *cmd, struct volume_group *vg, struct logical_volume *lv);
int select_match_pv(struct cmd_context *cmd, struct volume_group *vg, struct physical_volume *pv);
