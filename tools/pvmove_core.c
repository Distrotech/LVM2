...
/*
 * Update metadata for pvmove polling. Doesn't cover the inital
 * setup of pvmove
 *
...
{
	/*
	 * okozina FIXME: you broke cluster awareness, fix it
	 * see _pvmove_update_metadata_first in pvmove.c
	 */
	int r = 0;
	...
	if (!vg_commit(vg)) {
		...
		if (!resume_lv(cmd, lv_mirr)) {
			log_error("Unable to reactivate logical volume \"%s\"", lv_mirr->name);
			stack;
		}
		if (!revert_lv(cmd, lv_mirr))
			stack;
