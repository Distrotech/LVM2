...
/*
 * Called to set up initial pvmove LV.
 * (Not called after the last section completes.)
 */
static int _pvmove_update_metadata_first(struct cmd_context *cmd,
					  struct volume_group *vg,
					  struct logical_volume *lv_mirr,
					  struct dm_list *lvs_changed,
					  unsigned flags)
{
	...
	/* okozina FIXME: transfer _exclusive_ variable to new polldaemon */
			  ...
			  int argc, char **argv, union lvid *lvid)
{
	...
	if (flags & PVMOVE_FIRST_TIME) {
		...
		if (!_pvmove_update_metadata_first
