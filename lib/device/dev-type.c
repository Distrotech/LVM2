 ...
 */
static struct dev_known_type _dev_known_types[] = {
	{"ide", 64},		/* IDE disk */
	{"sd", 16},		/* SCSI disk */
...
struct dev_type_register *create_dev_type_register(const char *proc_dir,
						   const struct dm_config_node *cn)
{
	struct dev_type_register *dtr;
	char line[80];
	...
	if (!(dtr = dm_zalloc(sizeof(struct dev_type_register)))) {
	...
	if (!*proc_dir) {
		...
		for (i = 0; i < NUMBER_OF_MAJORS; i++)
			dtr->dev_types[i].max_partitions = 1;
		return dtr;
	}
	...
	while (fgets(line, sizeof(line), pd) != NULL) {
		...
		for (j = 0; _dev_known_types[j].name[0]; j++) {
			...
			if (dev_len <= strlen(line + i) &&
				...
				dtr->dev_types[line_maj].max_partitions =
		...
		for (cv = cn->v; cv; cv = cv->next) {
			...
			if (dev_len <= strlen(line + i) &&
			    ...
			    (line_maj < NUMBER_OF_MAJORS)) {
				dtr->dev_types[line_maj].max_partitions = cv->v.i;
				break;
			}
		}
	}
...
int dev_subsystem_part_major(struct dev_type_register *dtr, struct device *dev)
{
	dev_t primary_dev;

	if (MAJOR(dev->dev) == dtr->device_mapper_major)
		return 1;

	if (MAJOR(dev->dev) == dtr->drbd_major)
		return 1;

	if (MAJOR(dev->dev) == dtr->emcpower_major)
		return 1;

	if (MAJOR(dev->dev) == dtr->power2_major)
		return 1;

	if ((MAJOR(dev->dev) == dtr->blkext_major) &&
	    (dev_get_primary_dev(dtr, dev, &primary_dev) > 0) &&
	    (MAJOR(primary_dev) == dtr->md_major))
		return 1;

	return 0;
}

const char *dev_subsystem_name(struct dev_type_register *dtr, struct device *dev)
{
	if (MAJOR(dev->dev) == dtr->md_major)
		return "MD";
...
int major_max_partitions(struct dev_type_register *dtr, int major)
{
	if (major >= NUMBER_OF_MAJORS)
		return 0;

	return dtr->dev_types[major].max_partitions;
}

int major_is_scsi_device(struct dev_type_register *dtr, int major)
{
	if (major >= NUMBER_OF_MAJORS)
		return 0;

	return (dtr->dev_types[major].flags & PARTITION_SCSI_DEVICE) ? 1 : 0;
}
...
static int _is_partitionable(struct dev_type_register *dtr, struct device *dev)
{
	int parts = major_max_partitions(dtr, MAJOR(dev->dev));
...
int dev_is_partitioned(struct dev_type_register *dtr, struct device *dev)
{
	if (!_is_partitionable(dtr, dev))
		return 0;
 ...
 *   -1 on error
 */
int dev_get_primary_dev(struct dev_type_register *dtr,
			struct device *dev, dev_t *result)
{
	 ...
	 */
	if ((parts = dtr->dev_types[major].max_partitions) > 1) {
...
static unsigned long _dev_topology_attribute(struct dev_type_register *dtr,
					     ...
					     struct device *dev)
{
	...
	if (stat(path, &info) == -1) {
		...
		if (dev_get_primary_dev(dtr, dev, &primary) < 0)
			return 0;
...
unsigned long dev_alignment_offset(struct dev_type_register *dtr, struct device *dev)
{
	return _dev_topology_attribute(dtr, "alignment_offset", dev);
}

unsigned long dev_minimum_io_size(struct dev_type_register *dtr, struct device *dev)
{
	return _dev_topology_attribute(dtr, "queue/minimum_io_size", dev);
}

unsigned long dev_optimal_io_size(struct dev_type_register *dtr, struct device *dev)
{
	return _dev_topology_attribute(dtr, "queue/optimal_io_size", dev);
}

unsigned long dev_discard_max_bytes(struct dev_type_register *dtr, struct device *dev)
{
	return _dev_topology_attribute(dtr, "queue/discard_max_bytes", dev);
}

unsigned long dev_discard_granularity(struct dev_type_register *dtr, struct device *dev)
{
	return _dev_topology_attribute(dtr, "queue/discard_granularity", dev);
}
...
unsigned long dev_discard_granularity(struct dev_type_regiser *dtr, struct device *dev)
{
	return 0UL;
}
