...
static struct physical_volume *_pv_read(struct cmd_context *cmd,
					...
					uint32_t log_flags, int scan_label_only);
			 ...
			 struct physical_volume *pv)
{
	...
	struct pv_list *pvl;
	uint32_t log_flags = WARN_PV_READ | WARN_INCONSISTENT;
	int r = 0, consistent = 0;

	if (!(vg = vg_read_internal(fmt->cmd, vg_name, vgid, log_flags, &consistent))) {
				     ...
				     int *consistent, unsigned precommitted)
{
	...
	if (is_orphan_vg(vgname)) {
		...
		*consistent = 1;
		return _vg_read_orphans(cmd, log_flags, vgname);
	}
			     ...
			     const char *vgid, uint32_t log_flags, int *consistent)
{
	...
	if (!(vg = _vg_read(cmd, vgname, vgid, log_flags, consistent, 0)))
		return NULL;
	...
	if (log_flags & WARN_INCONSISTENT)
...
static int _get_pvs(struct cmd_context *cmd, uint32_t log_flags,
		struct dm_list *pvslist, struct dm_list *vgslist)
{
	...
	dm_list_iterate_items(strl, vgids) {
		...
		log_flags |= WARN_INCONSISTENT;

		if (!(vg = vg_read_internal(cmd, vgname, (!vgslist) ? vgid : NULL, log_flags, &consistent))) {
			stack;
			continue;
		}
...
{
	return _get_pvs(cmd, log_flags, NULL, NULL);
}
			       ...
			       uint64_t status_flags, uint32_t misc_flags)
{
	...
	log_flags = WARN_PV_READ;
	if (consistent)
		log_flags |= WARN_INCONSISTENT;

	/* If consistent == 1, we get NULL here if correction fails. */
	if (!(vg = vg_read_internal(cmd, vg_name, vgid, log_flags, &consistent))) {
