...
{
	struct dev_type_register *dtr = (struct dev_type_register *) f->private;
	const char *name;
	...
	if (!major_is_scsi_device(dtr, MAJOR(dev->dev)))
		return 0;

	switch (dev_get_primary_dev(dtr, dev, &primary_dev)) {
		case -1:
			/* Error. */
	...
	if (major != dtr->device_mapper_major) {
		log_error("mpath major %d is not dm major %d.", major,
			  dtr->device_mapper_major);
		return 0;
	}
...
struct dev_filter *mpath_filter_create(struct dev_type_register *dtr)
{
	const char *sysfs_dir = dm_sysfs_dir();
	struct dev_filter *f;
...
struct dev_filter *mpath_filter_create(struct device_type_register *dtr)
{
	return NULL;
}
