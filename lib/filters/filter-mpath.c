...
static int _udev_dev_is_mpath(struct device *dev)
{
	...
	value = udev_device_get_property_value((struct udev_device *)dev->aux_status.handle, "DM_MULTIPATH_DEVICE_PATH");
...
static int _ignore_mpath(struct dev_filter *f, struct device *dev)
{
	if (_dev_is_mpath(f, dev) == 1) {
		log_debug_devs("%s: Skipping mpath component device [aux:%s/%p]",
				dev_name(dev),
