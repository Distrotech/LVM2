				   ...
				   uint32_t vgstatus)
{
	...
	struct dev_type_register *dtr = fmt->cmd->dev_type_register;
	struct label *label;
	...
	if (!(existing = lvmcache_info_from_pvid(pvid_s, 0)) &&
		...
		if (existing->dev != dev) {
			...
			if (dev_subsystem_part_major(dtr, existing->dev) &&
			    !dev_subsystem_part_major(dtr, dev)) {
				log_very_verbose("Ignoring duplicate PV %s on "
						 "%s - using %s %s",
						 pvid, dev_name(dev),
						 dev_subsystem_name(dtr, existing->dev),
				...
				log_very_verbose("Ignoring duplicate PV %s on "
						 ...
						 dev_name(existing->dev));
				return NULL;
			} else if (!dev_subsystem_part_major(dtr, existing->dev) &&
				   dev_subsystem_part_major(dtr, dev))
