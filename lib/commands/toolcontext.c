...
static struct dev_filter *_init_filter_components(struct cmd_context *cmd)
{
	...
	if (!(filters[nr_filt] = lvm_type_filter_create(cmd->dev_type_register))) {
	...
	if (find_config_tree_bool(cmd, devices_md_component_detection_CFG)) {
		init_md_filtering(1);
		if ((filters[nr_filt] = md_filter_create(cmd->dev_type_register)))
			nr_filt++;
	}

	/* mpath component filter. Optional, non-critical. */
	if (find_config_tree_bool(cmd, devices_multipath_component_detection_CFG)) {
		if ((filters[nr_filt] = mpath_filter_create(cmd->dev_type_register)))
			nr_filt++;
	}
...
static int _init_filters(struct cmd_context *cmd, unsigned load_persistent_cache)
{
	...
	if (!(f4 = persistent_filter_create(cmd->dev_type_register, f3, dev_cache))) {
				       ...
				       unsigned threaded)
{
	...
	if (!(cmd->dev_type_register = create_dev_type_register(cmd->proc_dir,
...
static void _destroy_dev_type_register(struct cmd_context *cmd)
{
	if (!cmd->dev_type_register)
		return;

	dm_free(cmd->dev_type_register);
	cmd->dev_type_register = NULL;
}
...
int refresh_toolcontext(struct cmd_context *cmd)
{
	...
	dev_cache_exit();
	_destroy_dev_type_register(cmd);
	_destroy_tags(cmd);
