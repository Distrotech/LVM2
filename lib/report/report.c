				...
				struct generic_logical_volume *glv)
{
	...
	if (dm_snprintf(buf, buf_len, "%s%s", DEAD_LV_PREFIX, glv->dead->dname) < 0) {
			   ...
			   int full, int include_dead_lvs)
{
	...
	char buf[NAME_LEN + strlen(DEAD_LV_PREFIX) + 1];

	if (glv.is_dead) {
		if (full && glv.dead->indirect_origin)
			glv = *glv.dead->indirect_origin;
	...
	if (orig_p != glv.live) {
		if (!(ancestor_str = _get_glv_str(buf, sizeof(buf), &glv)))
			return_0;
		if (!glv.is_dead || include_dead_lvs) {
			if (!_str_list_append(ancestor_str, ancestors))
				return_0;
		}
		if (!_find_ancestors(ancestors, glv, full, include_dead_lvs))
			return_0;
	}
			   ...
			   const void *data, void *private)
{
	...
	if (!_find_ancestors(&ancestors, glv, 0, cmd->include_dead_entities)) {
				 ...
				 const void *data, void *private)
{
	...
	if (!_find_ancestors(&full_ancestors, glv, 1, cmd->include_dead_entities)) {
