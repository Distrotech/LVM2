...
#define ADD_TIME_PROP(id, flags) [id] = {id, flags},

static const struct time_prop _time_props[] = {
	ADD_TIME_PROP(TIME_NULL,                    0)
	ADD_TIME_PROP(TIME_NUM,                     0)
	ADD_TIME_PROP(TIME_STR,                     0)

	ADD_TIME_PROP(TIME_NUM_MULTIPLIER,          0)
	ADD_TIME_PROP(TIME_NUM_MULTIPLIER_NEGATIVE, 0)
	ADD_TIME_PROP(TIME_NUM_DAY,                 TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_NUM_YEAR,                TIME_PROP_DATE | TIME_PROP_ABS)

	ADD_TIME_PROP(TIME_STR_TIMEZONE,            TIME_PROP_TIME | TIME_PROP_ABS)

	ADD_TIME_PROP(TIME_FRAME_AGO,               TIME_PROP_DATE | TIME_PROP_TIME | TIME_PROP_REL)

	ADD_TIME_PROP(TIME_LABEL_DATE_TODAY,        TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_LABEL_DATE_YESTERDAY,    TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_WEEKDAY_SUNDAY,          TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_WEEKDAY_MONDAY,          TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_WEEKDAY_TUESDAY,         TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_WEEKDAY_WEDNESDAY,       TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_WEEKDAY_THURSDAY,        TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_WEEKDAY_FRIDAY,          TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_WEEKDAY_SATURDAY,        TIME_PROP_DATE | TIME_PROP_ABS)

	ADD_TIME_PROP(TIME_LABEL_TIME_NOON,         TIME_PROP_TIME | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_LABEL_TIME_MIDNIGHT,     TIME_PROP_TIME | TIME_PROP_ABS)

	ADD_TIME_PROP(TIME_UNIT_SECOND,             TIME_PROP_TIME | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_UNIT_SECOND_REL,         TIME_PROP_TIME | TIME_PROP_REL)
	ADD_TIME_PROP(TIME_UNIT_MINUTE,             TIME_PROP_TIME | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_UNIT_MINUTE_REL,         TIME_PROP_TIME | TIME_PROP_REL)
	ADD_TIME_PROP(TIME_UNIT_HOUR,               TIME_PROP_TIME | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_UNIT_HOUR_REL,           TIME_PROP_TIME | TIME_PROP_REL)
	ADD_TIME_PROP(TIME_UNIT_AM,                 TIME_PROP_TIME | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_UNIT_PM,                 TIME_PROP_TIME | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_UNIT_DAY,                TIME_PROP_DATE | TIME_PROP_REL)
	ADD_TIME_PROP(TIME_UNIT_WEEK,               TIME_PROP_DATE | TIME_PROP_REL)
	ADD_TIME_PROP(TIME_UNIT_MONTH,              TIME_PROP_DATE | TIME_PROP_REL)
	ADD_TIME_PROP(TIME_UNIT_YEAR,               TIME_PROP_DATE | TIME_PROP_REL)
	ADD_TIME_PROP(TIME_UNIT_DECADE,             TIME_PROP_DATE | TIME_PROP_REL)
	ADD_TIME_PROP(TIME_UNIT_TZ_MINUTE,          TIME_PROP_TIME | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_UNIT_TZ_HOUR,            TIME_PROP_TIME | TIME_PROP_ABS)

	ADD_TIME_PROP(TIME_MONTH_JANUARY,           TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_MONTH_FEBRUARY,          TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_MONTH_MARCH,             TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_MONTH_APRIL,             TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_MONTH_MAY,               TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_MONTH_JUNE,              TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_MONTH_JULY,              TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_MONTH_AUGUST,            TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_MONTH_SEPTEMBER,         TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_MONTH_OCTOBER,           TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_MONTH_NOVEMBER,          TIME_PROP_DATE | TIME_PROP_ABS)
	ADD_TIME_PROP(TIME_MONTH_DECEMBER,          TIME_PROP_DATE | TIME_PROP_ABS)
};
...
static const struct time_reg _time_reg[] = {
	...
	{"decade",    TIME_PROP(TIME_UNIT_DECADE),          TIME_REG_PLURAL_S},
...
static void _adjust_date_time_granularity(struct time_info *info, struct time_item *ti)
{
	struct time_item *ti_p;
	time_id_t gran_date;
	time_id_t gran_time;
	int skip, is_abs, is_rel;

	is_abs = ti->prop->prop_flags & TIME_PROP_ABS;
	is_rel = ti->prop->prop_flags & TIME_PROP_REL;

	skip = 0;
	if (ti->prop->id == TIME_NUM_YEAR)
		gran_date = TIME_UNIT_YEAR;
	else if (_is_time_month(ti->prop->id))
		gran_date = TIME_UNIT_MONTH;
	else if (ti->prop->id == TIME_NUM_DAY)
		gran_date = TIME_UNIT_DAY;
	else
		skip = 1;

	if (!skip) {
		if (is_abs) {
			if (gran_date > info->max_abs_date_granularity)
				info->max_abs_date_granularity = gran_date;
			if (gran_date < info->min_abs_date_granularity)
				info->min_abs_date_granularity = gran_date;
		}
	}

	skip = 0;
	if (_is_time_unit(ti->prop->id))
		gran_time = ti->prop->id;
	else if ((ti->prop->id == TIME_NUM_MULTIPLIER) &&
		(ti_p = (struct time_item *) dm_list_next(info->ti_list, &ti->list)) &&
		((ti_p->prop->id == TIME_UNIT_AM) || (ti_p->prop->id == TIME_UNIT_PM))) {
		is_abs = 1;
		is_rel = 0;
		gran_time = TIME_UNIT_HOUR;
	} else
		skip = 1;

	if (!skip) {
		if (is_abs && (gran_time < info->min_abs_time_granularity))
			info->min_abs_time_granularity = gran_time;
		else if (is_rel && (gran_time < info->min_rel_time_granularity))
			info->min_rel_time_granularity = gran_time;
	}
}
...
static int _recognize_time_items(struct time_info *info)
{
	 ...
	 * given by strings we recognized before. Also, get granularities.
	 */
	dm_list_iterate_items(ti, info->ti_list) {
		...
		_adjust_date_time_granularity(info, ti);
	}
...
static time_t *_get_now(struct dm_report *rh, struct dm_pool *mem)
{
	...
	if (!(cached_obj = dm_report_reserved_cache_get(rh, CACHE_ID_TIME_NOW))) {
		if (!(now = dm_pool_zalloc(mem, sizeof(time_t)))) {
			log_error("_get_now: dm_pool_zalloc failed");
			return NULL;
		}
		time(now);
		if (!dm_report_reserved_cache_set(rh, CACHE_ID_TIME_NOW, now)) {
				 ...
				 const void *data_out)
{
	...
	time_t *now = _get_now(rh, info->mem);
	long multiplier = 1;
	struct tm tm_now;
	time_id_t id;
	char *end;
	...
	log_error(">>> RELATIVE TIME: @%ld", t);
	log_error(">>> ABSOLUTE TIME: %d-%02d-%02d  %02d:%02d:%02d", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);

	t = mktime(&tm) - t;
	log_error("RESULT: @%ld", t);
	return 0;
}

static void *_lv_time_handler_parse_fuzzy_name(struct dm_report *rh,
					       struct dm_pool *mem,
					       const void *data_in)
{
	const char *s = (const char *) data_in;
	void *data_out = NULL;
				 ...
				 const void *data, void *private)
{
	...
	if (!_find_ancestors(&full_ancestors, lv, 1)) {
				   ...
				   const void *data, void *private)
{
	...
	if (!_find_descendants(&full_descendants, lv, 1)) {
		dm_pool_free(full_descendants.mem, full_descendants.result);
		return_0;
	}
